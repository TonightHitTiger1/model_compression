        ...
        def quantize_weights_model(model):
            for idx, p in enumerate(model.parameters()):
                ...
                if backprop_quantization_style in ('none', 'truncated'):
                    #Modify Here111111111
                    if len(p.data.size()) == 4:
                        for i in range(len(p.data)):
                            if p.data[i].cpu().numpy().sum() == 0:
                                continue
                            p.data[i] = quantizeFunctions(p.data[i])
                    # print("before p-data[0][0]",p.data[0])
                    else:
                        p.data = quantizeFunctions(p.data)
                        # print("after p-data[0][0]", p.data[0])m
                        # print("zhixing111")
                        # sys.exit(1)
                elif backprop_quantization_style == 'complicated':
...
def optimize_quantization_points(modelToQuantize, train_loader, test_loader, initial_learning_rate = 1e-5,
    ...
    # for _ in modelToQuantize.parameters():
    #     print("===:",_)
    # print("numTensorNetwork:",numTensorsNetwork)
    # sys.exit(1)
