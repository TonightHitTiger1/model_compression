...
def train(model, loss_fn, optimizer, param, loader_train, loader_val=None):
    ...
    for epoch in range(param['num_epochs']):
        ...
        for t, (x, y) in enumerate(loader_train):
            ...
            if (t + 1) % 100 == 0:
                # print("test2 = ",loss.data.cpu().numpy())
...
def test(model, loader):
    ...
    for x, y in loader:   # x : test x,  y:test y  xunhuan 100 ci
        inputs = to_var(x, volatile=True)
...
def prune_rate(model, verbose=True):
    ...
    for parameter in model.parameters():
        ...
        if len(parameter.data.size()) != 1:
            ...
            if verbose:
                print("Layer {} | {} layer | {:.2f}% parameters pruned--param nums this layer={}" \
                    .format(
                        layer_id,
